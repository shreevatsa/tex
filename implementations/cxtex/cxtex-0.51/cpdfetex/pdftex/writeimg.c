...
$Id: writeimg.c,v 1.2 2004/05/11 14:30:32 taco Exp $
*/
...
{
    return pdf_pntr(img)->orig_x;
}

integer epdforigy(integer img)
{
    return pdf_pntr(img)->orig_y;
}
...
integer readimage(str_number s, integer page_num, str_number page_name,
                  ...
                  integer pdfoptionpdfinclusionerrorlevel)
{
	...
	*q = (*p == '"' ? *(++p) : *p);
    ...
    switch (img_type(img)) {
    case IMAGE_TYPE_PDF:
        pdf_pntr(img) = xtalloc(1, pdf_image_struct);
        pdf_pntr(img)->page_box = pdflastpdfboxspec;
        pdf_pntr(img)->always_use_pdfpagebox = pdfoptionalwaysusepdfpagebox;
        ...
        pdf_pntr(img)->orig_x = bp2int(epdf_Orig_x);
        pdf_pntr(img)->orig_y = bp2int(epdf_Orig_y);
        pdf_pntr(img)->selected_page = page_num;
        pdf_pntr(img)->doc = epdf_doc;
        break;
    case IMAGE_TYPE_PNG:
        img_pages(img) = 1;
        read_png_info(img);
...
void writeimage(integer img)
{
    ...
    case IMAGE_TYPE_JPG:
        ...
        break;
    case IMAGE_TYPE_PDF:
        epdf_doc = pdf_pntr(img)->doc;
        epdf_selected_page = pdf_pntr(img)->selected_page;
        epdf_page_box = pdf_pntr(img)->page_box;
        epdf_always_use_pdf_pagebox = pdf_pntr(img)->always_use_pdfpagebox;
        write_epdf();
        break;
    default:
        pdftex_fail("unknown type of image");
