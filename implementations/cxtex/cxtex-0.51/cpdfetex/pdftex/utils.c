...
void ff_flush(void)
{
    ...
    for (p = ff_buf_tab; p < ff_buf_ptr;) {
        n = pdf_buf_size - pdfptr;
        if (ff_buf_ptr - p < n)
            n = ff_buf_ptr - p;
        memcpy(pdfbuf + pdfptr, p, (unsigned)n);
        pdfptr += n;
        if (pdfptr == pdf_buf_size)
            pdfflush();
        p += n;
    }
...
void make_subset_tag(fm_entry *fm_cur, integer fn_offset)
{
    char tag[7];
    unsigned long crc;
    eight_bits *fontname_ptr = (eight_bits*)ff_buf_tab + fn_offset;
...
str_number maketexstring(const char *s)
{
    ...
    if (s == 0 || *s == 0)
        return get_nullstr();
    l = strlen(s);
    check_buf(pool_ptr + l, pool_size);
    while (l-- > 0)
        str_pool[pool_ptr++] = *s++;
...
void tex_printf(const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    vsprintf(print_buf, fmt, args);
    zprint(maketexstring(print_buf));
...
char *makecstring(integer s)
{
    ...
    char *p = cstrbuf;
    int i, l = str_start[s + 1] - str_start[s];
...
boolean str_eq_cstr(str_number n, char *s)
{
    int l;
    if (s == 0 || n == 0)
        return false;
    l = str_start[n];
    while (*s && l < str_start[n + 1] && *s == str_pool[l])
        l++, s++;
    return !*s && l == str_start[n + 1];
}
...
void setjobid(int Year, int Month, int Day, int Time, int pdftexversion, int pdftexrevision)
{
    ...
    pdftex_banner = maketexstring(BANNER);
...
str_number getresnameprefix(void)
{
 ...
 */
void printID (str_number filename)
{
    time_t t;
    size_t size;
    char time_str[32];
