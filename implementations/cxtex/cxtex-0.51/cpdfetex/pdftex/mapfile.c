...
$Id: mapfile.c,v 1.2 2004/05/11 14:30:32 taco Exp $
*/
...
internal_font_number tfmoffm(integer i)
{
    return fm_tab[i].tfm_num;
}

void checkextfm(str_number s, integer e)
{
    char *p;
    if (e == 0)
        return;
...
static fm_entry *mk_ex_fm(internal_font_number f, int fm_index, int ex)
{
    fm_entry *e;
    fm_new_entry();
    e = fm_tab + fm_index;
    ...
    fm_ptr->tfm_name = xstrdup(makecstring(font_name[f]));
    ...
    for (p = fm_tab; p < fm_ptr; p++)
        if (p->ps_name != 0 && strcmp(p->ps_name, bname) == 0) {
            if (is_basefont(p) || is_noparsing(p) || !is_included(p))
                return -1;
            if (p->tfm_num == get_null_font()) {
                if (p->tfm_name == nontfm)
                    p->tfm_num = new_null_font();
                else
                    p->tfm_num = get_tfm_num(maketexstring(p->tfm_name));
            }
            i = p->tfm_num;
            if (pdffontmap[i] == -1)
...
static void init_fm(fm_entry *fm, internal_font_number f)
{
    if (fm->fd_objnum == 0)
        fm->fd_objnum = pdfnewobjnum();
    if (fm->ff_objnum == 0 && is_included(fm))
        fm->ff_objnum = pdfnewobjnum();
    if (fm->tfm_num == get_null_font())
        fm->tfm_num = f;
}

integer fmlookup(internal_font_number f)
{
    char *tfm, *p;
    fm_entry *fm, *exfm;
    int e, l;
