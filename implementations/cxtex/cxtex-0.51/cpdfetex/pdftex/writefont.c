...
static void print_key(integer code, integer v)
{
    ...
    if (!font_keys[code].valid) {
        pdf_printf("%i",
                   ...
                   (int)divide_scaled(v, pdffontsize[tex_font], 3));
    }
    else 
...
{
    return -atan(get_slant(tex_font)/65536.0)*(180/M_PI);
}

static integer getstemv(void)
{
    return get_char_width(tex_font, '.')/3;
}
...
static void getbbox(void)
{
    ...
    font_keys[FONTBBOX2_CODE].value.num = 
        divide_scaled(-get_char_depth(tex_font, 'y'), pdffontsize[tex_font], 3);
    font_keys[FONTBBOX3_CODE].value.num =
        divide_scaled(get_quad(tex_font), pdffontsize[tex_font], 3);
    font_keys[FONTBBOX4_CODE].value.num =
        divide_scaled(get_char_height(tex_font, 'H'), pdffontsize[tex_font], 3);
}
 ...
 * when font_ec[f] < 128 
 */
void update_enc(internal_font_number f, char **glyph_names)
{
    int i;
...
static void get_char_widths(void)
{
    ...
    for (i = 0; i <= MAX_CHAR_CODE; i++) {
        if (i < font_bc[tex_font] || i > font_ec[tex_font])
            ...
            char_widths[i] = get_char_width(tex_font, i);
    }
    for (i = font_bc[tex_font]; i <= 32; i++)
        char_widths[pdfcharmap[tex_font][i]] = char_widths[i];
    for (i = font_bc[tex_font]; i <= MAX_CHAR_CODE; i++)
    ...
    for (i = first_char; i <= last_char; i++)
        if (pdfcharmarked(tex_font, i))
            char_widths[i] = divide_scaled(char_widths[i], 
...
{
    int i;
    pdf_begin_obj(char_widths_objnum); 
    pdf_puts("[");
    for (i = first_char; i <= last_char; i++)
...
static void write_fontfile(void)
{
    ...
    if (fm_cur->ff_objnum == 0)
        pdftex_fail("font file object number for `%s' not initialized",
                    fm_cur->tfm_name);
    pdf_begin_dict(fm_cur->ff_objnum); /* font file stream */
    if (is_truetype(fm_cur))
...
static void write_fontdescriptor(void)
{
    ...
    pdf_begin_dict(fm_cur->fd_objnum); /* font descriptor */
    print_key(ASCENT_CODE, get_char_height(tex_font, 'h'));
    print_key(CAPHEIGHT_CODE, get_char_height(tex_font, 'H'));
    print_key(DESCENT_CODE, -get_char_depth(tex_font, 'y'));
    pdf_printf("/FontName ");
    write_fontname(true);
    print_key(ITALIC_ANGLE_CODE, getitalicangle());
    print_key(STEMV_CODE, getstemv());
    print_key(XHEIGHT_CODE, get_x_height(tex_font));
...
void dopdffont(integer font_objnum, internal_font_number f)
{
    int i;
    tex_font = f;
    cur_glyph_names = 0;
    ...
    if (pdffontmap[tex_font] == -1)
        pdftex_fail("pdffontmap not initialized for font %s", 
                    makecstring(font_name[tex_font]));
...
/*         if (!is_truetype(fm_cur) || !indexed_enc(fm_cur)) { */
    ...
    if (!is_basefont(fm_cur))
        write_fontfile();
    if (fm_cur->fn_objnum != 0) {
        pdf_begin_obj(fm_cur->fn_objnum);
