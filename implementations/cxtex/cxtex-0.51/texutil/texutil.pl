...
#TH This forces predeclared variables.
use strict; #TH
no strict 'refs'; #TH
...
my $Program = "TeXUtil 8.1f - ConTeXt / PRAGMA ADE 1992-2003" ;
...
my $ProcessReferences;
my $ProcessIJ;
my $ProcessHigh;
my $ProcessQuotes;
my $ProcessDocuments;
my $ProcessType="";
my $ProcessOutputFile;
my $ProcessSources;
my $ProcessSetups;
my $ProcessTemplates;
my $ProcessInfos;
...
#TH Old program logic said: IF Not Silent THEN DisableVerbose
#TH The effect of that is that the only way to
#TH have verbose reporting, is to say:
#TH
#TH > texutil --silent --verbose
#TH
#TH which enables verbose reporting, but to the log file only.
#TH
#TH Just
#TH
#TH > texutil --verbose
#TH
#TH didn't actually work at all, which I consider confusing
...
my $InputFile = "@ARGV" ; # niet waterdicht
...
sub CompFileName
  { my ($a,$b) = @_ ;
    ...
    if (($sa =~ /^\d+$/o)&&($sb =~ /^\d+$/o))
      { $a = $fa . "." . sprintf("%10d",$sa) ;
        $b = $fb . "." . sprintf("%10d",$sb) ;
		#TH next replacements should be global I think.
		$a =~ s/\s/0/og ; #TH+/g
		$b =~ s/\s/0/og ; #TH+/g
	  }
...
sub SetOutputFile
  { my ($OutFilNam, $OutFilSuf) = split (/\./, $_[0], 2) ;
...
sub SplitFileName
 { my $Rubish = "" ;
   if ($_[0] =~ /^\.\//)
     ...
     { $FileName = $_[0] }
   #TH Next line would split "file.tex.org" into ("file", "tex.org")
   # return split (/\./, $FileName, 2); #TH
   #TH new code splits to ("file.tex","org")
   if ($FileName =~ /^(.*)\.(.*)$/) {   #TH
	 return ($1,$2);                    #TH
   }                                    #TH
   return ($FileName,"");               #TH
...
if ($UserInterface eq "nl")
  ...
  { # begin of dutch section
	...
	#TH next was missing
    $MS{"ExtraProgram"}            = "       extra programma :" ; #TH
...
sub HandleKey
  { my $RestOfLine = shift; #TH
	++$SortN ;
    $RestOfLine =~ s/\{(.*)\}/$1/o ;
    my ($lan, $enc, $str, $chr, $map, $alf) = split(/\}\s*\{/, $RestOfLine) ;
    if ($str =~ /^(\d+)/) { $str = ''.chr($Filter[$1]) }
    #TH Next fix is a guess. Original was:
    #  $map = chr(ord($MAP[$i])+128) ; #TH
	$map = chr(ord($MAP[$SortN])+128) ; #TH
	#TH end fix
    $STR[$SortN] = $str ;
    $CHR[$SortN] = $chr ;
    $MAP[$SortN] = $map ;
...
#TH Next line is superfluous because of the replacement tagged as '# ?', below
#TH
# $string =~ s/\\getXMLentity\s*\{(.*?)\}/$1/gio ;  # {tex} => tex #TH
...
sub InitializeCommands
  { #TH The TUO is not open yet when this function is called ..
	# print TUO "%\n" . "% $Program / Commands\n" . "%\n" ; #TH
    $NOfCommands = 0 }
...
sub HandleCommand
  { my $RestOfLine = shift; #TH
    ++$NOfCommands ;
    $RestOfLine =~ s/^\\//;
    #TH next 2 regexes used /i and/or /o, but shouldnt
    if ($RestOfLine =~ /^pospxy/o)
      { ++$NOfPositionsFound }
    elsif ($RestOfLine =~ /^initializevariable\\totalnofpositions\{(.*)\}/) #TH/i
      { $TotalNOfPositions = $1 }
    elsif ($RestOfLine =~ /^initializevariable\\totalnofMPgraphics\{(.*)\}/) #TH/i
...
sub HandleExtra
  { my $RestOfLine = shift; #TH
	if ($RestOfLine =~ /(.)\s+(.*)\s*$/o)
      { if ($1 eq "p")
          { #TH next 1 regexes used /i and/or /o, but shouldnt
			my $str = $2 ; $str =~ s/^\{(.*)\}$/$1/ ; #TH/o
...
sub HandlePlugIn
  { my $RestOfLine = shift; #TH
	#TH next 2 regexes used /i and/or /o, but shouldnt
	if ($RestOfLine =~ /\s*u\s*\{(.*?)\}\s*(.*)\s*/o) #TH/i
      { my $tag = $1 ;
        my $arg = $2 ;
...
sub HandleSynonym
  { my $RestOfLine = shift; #TH
    ...
    my ($Class, $Key, $Entry, $Meaning) = split(/} \{/, $RestOfLine) ;
    chop $Meaning ;
...
sub FlushSynonyms
  { print TUO "%\n" . "% $Program / Synonyms\n" . "%\n" ;
    ...
    my $NOfSaneSynonyms = 0 ;
    for (my $n=1; $n<=$NOfSynonyms; $n++)
      { # check normally not needed
        if (($n==1)||($SynonymEntry[$n] ne $SynonymEntry[$n-1]))
          { my ($Class, $Key, $Entry, $Meaning) =
...
my $SortMethod; #TH pointless variable (?)
...
sub HandleRegister # the } { makes sure that local {} is ok
  { my $RestOfLine = shift; #TH
	...
	($SecondTag, $RestOfLine) = split(/ /, $RestOfLine, 2) ;
    ++$NOfEntries ;
	my ($Class, $Location, $Key, $Entry, $SeeToo, $Page, $RealPage);
    #~ if ($SecondTag eq "s")
   ...
   if ($SecondTag eq "s")
    ...
    my $LCKey = lc $Key ;
    my $RegStatus = $RegStat{$SecondTag} ;
    my $RealPageNumber= sprintf("%6i",$RealPage) ;
...
my ($SavedFrom,$SavedTo, $SavedEntry);
...
sub FlushRegisters
  { print TUO "%\n" . "% $Program / Registers\n" . "%\n" ;
    ...
    for (my $n=1 ; $n<=$NOfEntries ; ++$n)
      { my ($Class, $LCKey, $Key, $Entry, $TextHow, $RegisterState,
           $RealPage, $Location, $Page, $PageHow, $SeeToo) =
             split(/$JOIN/, $RegisterEntry[$n]) ;
        $RealPage =~ s/^\s*//o ;
        my $TestAlfa = lc substr $Key, 0, 1 ;
        #
        if ($SortN)
          { my $AlfKey = $Key ;
#            $AlfKey =~ s/(.).*\x00(.).*/$1$2/o ;
        ...
        if ($RealPage eq 0)
          ...
          { # print "$LastPage / $Page // $LastRealPage / $RealPage\n" ;
            my $NextEntry = "{$Class}{$PreviousA}{$PreviousB}{$PreviousC}{$PageHow,$TextHow}" ;
            my $SavedLine = "{$Class}{$PageHow,$TextHow}{$Location}{$Page}{$RealPage}\n" ;
      ...
      { unless (open (TUI, "$InputFile.tui"))
          ...
          { Report("InputFile", "$InputFile.tui" ) ;
            ...
            while (<TUI>)
              { $SomeLine = $_ ;
                chomp $SomeLine ;
                my ($FirstTag, $RestOfLine) = split ' ', $SomeLine, 2 ;
                if    ($FirstTag eq "c")
                  { HandleCommand($RestOfLine) }   #TH + argument
                elsif ($FirstTag eq "s")
                  { HandleSynonym($RestOfLine) }   #TH + argument
                elsif ($FirstTag eq "r")
                  { HandleRegister($RestOfLine) }  #TH + argument
                elsif ($FirstTag eq "f")
                  { HandleFile($RestOfLine) }      #TH + argument
                elsif ($FirstTag eq "k")
                  { HandleKey($RestOfLine) }       #TH + argument
                elsif ($FirstTag eq "e")
                  { HandleExtra($RestOfLine) }     #TH + argument
                elsif ($FirstTag eq "p")
                  { HandlePlugIn($RestOfLine) }    #TH + argument
                elsif ($FirstTag eq "q")
                  { $ValidOutput = 0 ;
                    last } }
            if ($ValidOutput)
            ...
            unless (open (TEX, "$FileName.$FileSuffix"))
              ...
              { Report("InputFile",  "$FileName.$FileSuffix") ;
                ...
                while (<TEX>)
                  { $SomeLine = $_ ;
                    chomp $SomeLine ;
                    my ($Tag, $RestOfLine) = split(/ /, $SomeLine, 2) ;
                    if ($Tag eq "%S")
                      { ++$NOfSetups ;
                        while ($Tag eq "%S")
...
sub HandleEditorCues
  { if ($ProcessTemplates)
    ...
    if ($InputFile eq "")
      ...
      { CheckInputFiles ($InputFile) ;
        foreach my $FullName (@UserSuppliedFiles)
          { ($FileName, $FileSuffix) = SplitFileName ($FullName) ;
            if ($FileSuffix eq "")
              { $FileSuffix = "tex" }
            unless (-f "$FileName.$FileSuffix")
      ...
      { OpenTerminal ;
        if ($FUni)
         ...
         (sprintf "x=%1.3fcm y=%1.3fcm ", $FXof, $FYof) .
		   #TH I guess this should be the same as below
           ...
           while (<EPS>)
             { $SomeLine = $_;
               ...
               if (($BBoxFound) && ((substr $SomeLine,0,1) ne "%"))
                 { last }
               if ($BBoxFound<2)
				 #TH next 7 regexes used /i and/or /o, but shouldnt
                 { if ($SomeLine =~ /^%%BoundingBox:/o) #TH/i
                     { $EpsBBox = $SomeLine ; $BBoxFound = 1 ; next }
                   elsif ($SomeLine =~ /^%%HiResBoundingBox:/o) #TH/i
                     { $EpsBBox = $SomeLine ; $BBoxFound = 2 ; next }
                   elsif ($SomeLine =~ /^%%ExactBoundingBox:/o) #TH/i
                     { $EpsBBox = $SomeLine ; $BBoxFound = 3 ; next } }
               if ($SomeLine =~ /^%%PDFready:/o) #TH/i
                 { $PDFReady = 1 }
               elsif ($SomeLine =~ /^%%Creator:/o)  #TH/i
                 { my $Tag; ($Tag, $EpsCreator) = split (/ /, $SomeLine, 2) ;
                   if ($EpsCreator =~ /MetaPost/o) #TH/i
                     { $MPSFound = 1 } }
               elsif ($SomeLine =~ /^%%Title:/o) #TH/i
                 { my $Tag; ($Tag, $EpsTitle) = split (/ /, $SomeLine, 2) } }
           close ( EPS ) ;
           if ($BBoxFound)
             { my ($Tag, $LLX, $LLY, $URX, $URY, $RestOfLine) = split (/ /, $EpsBBox, 6 ) ;
               my $EpsHeight  = ($URY-$LLY)*$DPtoCM ;
               my $EpsWidth   = ($URX-$LLX)*$DPtoCM ;
               my $EpsXOffset = $LLX*$DPtoCM ;
               my $EpsYOffset = $LLY*$DPtoCM ;
			   my $EpsType;
               if ($MPSFound)
                 { $EpsType = "mps" }
               else
...
sub HandlePdfFigure
  { my ( $SuppliedFileName ) = @_ ;
    ...
    while (<PDF>)
      { $SomeLine = $_ ;
        chomp ($SomeLine) ;
		#TH next 3 regexes used /i and/or /o, but shouldnt
        if ($SomeLine =~ /\/Type\s*\/Pages/o) #TH/i
          { $PagesFound = 1 }
        elsif ($SomeLine =~ /\/Type\s*\/Page/o) #TH/i
          { ++$PageFound ;
            if ($PageFound>1) { last } }
        if ((($PageFound)||($PagesFound)) && ($SomeLine =~ /\/MediaBox /o)) #TH/i
          { $MediaBox = $SomeLine ;
            $MediaBoxFound = 1 ;
            if ($PagesFound) { last } } }
    close ( PDF ) ;
...
#    elsif (($MediaBoxFound) && ($MediaBox))
    if (($MediaBoxFound) && ($MediaBox))
      { my $D = "[0-9\-\.]" ;
        ...
        my $LLX = $1 ; my $LLY = $2 ; my $URX = $3 ; my $URY = $4 ;
        my $PdfHeight = ($URY-$LLY)*$DPtoCM ;
        my $PdfWidth = ($URX-$LLX)*$DPtoCM ;
        my $PdfXOffset = $LLX*$DPtoCM ;
        my $PdfYOffset = $LLY*$DPtoCM ;
        SaveFigurePresets
         ( $PdfFileName, "pdf", 1,
...
sub TifGetAscii
  { #TH $S is a singular value, so don't do "my ($S) ..."
	my $S = "" ; #TH
    --$TifValues;
	#TH This was broken
    # if ($TifValues) #TH
...
my ($TifWidth,$TifHeight,$TifUnit,$TifHRes,$TifVRes);
my ($TifCreator,$TifAuthor,$TifTitle);

sub TifGetChunk
  { seek TIF, $TifNextChunk, 0 ;
    my $Length = TifGetShort ;
    $TifNextChunk += 2 ;
    for (my $i=1; $i<=$Length; $i++)
      { seek TIF, $TifNextChunk, 0 ;
        my $TifTag = TifGetShort ;
        my $TifType = TifGetShort ;
...
#TH Added the czech, italian, norwegian and romanian versions
my $Unknown = "onbekende verwijzing|"    .
              "unbekannte Referenz|"     .
              "unknown reference|"       .
              "neznama reference|"       . #TH
              "riferimento sconosciuto|" . #TH
              "ukjent referanse|"        . #TH
              "referinta necunoscuta|"   . #TH
              "dubbele verwijzing|"      .
              "duplicate reference|"     .
              "doppelte Referenz|"       .
              "duplicitni reference|"    . #TH
              "riferimento duplicato|"   . #TH
              "duplikat referanse|"      . #TH
              "referinta duplicat"       ; #TH
                      ...
                      { ++$NOfBoxes ;
                        ...
                        if ($SomePoints>=$ProcessCriterium)
                          { ++$NOfMatching ;
                            ...
                            print ALL $SomeLine } }
					#TH next 1 regexes used /i and/or /o, but shouldnt
                    if (($ProcessUnknown) && ($SomeLine =~ /$Unknown/o)) #TH/i
                     { ++$NOfUnknown ;
                       FlushLogTopic ;
                       print ALL "$SomeLine\n" } } } }
        close (ALL) ;
      ...
      { foreach (@files)
          { if (/(.*)\.$suffix$/i)
              { if ((-e "$1.tex")||(-e "$1.xml"))
                  ...
                  { $strippedname = $1 ;
					#TH This next line matched input-t.mp, but not input-mpgraph.mp
					#TH (matches a single character only)
					#   $strippedname =~ s/\-[a-z]$//oi ; #TH
					#TH I think you want this:
					$strippedname =~ s/\-[a-z]+$//oi ; #TH
